...
@st.cache_data(ttl=1800, show_spinner=False)
def calcular_aging_vectorized(inicio_series: pd.Series, fim_series: pd.Series, status_series: pd.Series) -> pd.Series:
...
@st.cache_data(ttl=1800, show_spinner=False)
def calcular_garantia_vectorized(dt_nf_series: pd.Series, garantia_dias_series: pd.Series) -> tuple:
    ...
    status_garantia = np.where(
        fim_garantia.notna(),
        np.where(fim_garantia >= hoje, 'DENTRO', 'FORA'),
        ''
    )
