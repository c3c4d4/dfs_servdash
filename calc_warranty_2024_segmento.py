...
def process_chunk(chunk, rtm_map, garantia_map):
    # Filter by Year first to reduce size
    if "DT_NUM_NF" not in chunk.columns:
        return None

    # We do NOT convert the whole chunk's date yet to avoid warnings/errors on bad rows
    # But we need to filter by SEGMENTO first
    ...
    mask_2024 = df["DT_NUM_NF"].dt.year == TARGET_YEAR
    df = df[mask_2024].copy()
