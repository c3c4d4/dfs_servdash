...
def get_ss_for_chassis(chassis_list: List[str], chamados_dict: Dict[str, List[str]]) -> Set[str]:
...
@st.cache_data(ttl=900, show_spinner=False)
def calculate_qtd_chamados(df: pd.DataFrame, chassi_counts_series: pd.Series) -> pd.Series:
...
def create_duracao_garantia_column(garantia_series: pd.Series) -> pd.Series:
    ...
    return garantia_series.apply(lambda x: 
        '6 meses (183 dias)' if x == GARANTIA_PERIODS['6_MESES'] else
        '12 meses (365 dias)' if x == GARANTIA_PERIODS['12_MESES'] else
        '18 meses (548 dias)' if x == GARANTIA_PERIODS['18_MESES'] else
        '24 meses (730 dias)' if x == GARANTIA_PERIODS['24_MESES'] else
        '36 meses (1095 dias)' if x == GARANTIA_PERIODS['36_MESES'] else
        'Outros' if pd.notna(x) else 'Não informado'
    )

def determine_partida_inicial_status(num_serial: str, partida_set: Set[str], qtd_chamados: int) -> str:
...
def calculate_kpi_percentages(filtered_df: pd.DataFrame, 
                            partida_set: Set[str], 
                            chassi_counts_validos: pd.Series,
                            chamados_garantia_chassis: Set[str]) -> Dict[str, float]:
    """Calculate all KPI percentages in one place."""
    
    if len(filtered_df) == 0:
        return {key: 0.0 for key in [
            'pct_com_partida_dfs', 'pct_com_partida_terceiros', 'pct_com_chamado',
            'pct_sem_chamado', 'pct_rtm', 'pct_em_garantia', 'pct_fora_garantia'
        ]}
    ...
    filtered_temp['QTD_CHAMADOS'] = calculate_qtd_chamados(filtered_temp, chassi_counts_validos)
    filtered_temp['PARTIDA_INICIAL'] = filtered_temp.apply(
        lambda row: determine_partida_inicial_status(row['NUM_SERIAL'], partida_set, row['QTD_CHAMADOS']), 
        axis=1
    )
    ...
    # Garantia status
    em_garantia_count = filtered_df[filtered_df['STATUS_GARANTIA'] == 'DENTRO']['NUM_SERIAL'].nunique()
    pct_em_garantia = 100 * em_garantia_count / total_bombas
    
    fora_garantia_count = filtered_df[filtered_df['STATUS_GARANTIA'] == 'FORA']['NUM_SERIAL'].nunique()
...
def calculate_garantia_distribution(df: pd.DataFrame) -> Dict[str, float]:
    ...
    try:
        if 'GARANTIA' not in df.columns or len(df) == 0:
            return {key: 0.0 for key in ['pct_6m', 'pct_12m', 'pct_18m', 'pct_24m', 'pct_36m']}
...
def calculate_rtm_values(erros_rtm_df: pd.DataFrame) -> Dict[str, float]:
    ...
    try:
        ...
        # Handle NaN values
        return {
            'media_valor_total': media_valor_total if pd.notna(media_valor_total) else 0.0,
...
def add_garantia_eletronica_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_copy['FIM_GARAN_ELETRICA'] = pd.to_datetime(df_copy['DT_NUM_NF'], errors='coerce') + timedelta(days=GARANTIA_ELETRONICA_DAYS)
    df_copy['FIM_GARAN_ELETRICA'] = df_copy['FIM_GARAN_ELETRICA'].dt.strftime('%d/%m/%Y')
    
    hoje = pd.Timestamp.now().normalize()
    fim_garan_eletrica_dt = pd.to_datetime(df_copy['FIM_GARAN_ELETRICA'], format='%d/%m/%Y', errors='coerce')
    df_copy['STATUS_GARAN_ELETRICA'] = ['DENTRO' if (pd.notnull(fim) and hoje <= fim) else 'FORA' for fim in fim_garan_eletrica_dt]
