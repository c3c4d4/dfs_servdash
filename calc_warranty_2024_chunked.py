...
def process_chunk(chunk, rtm_map, garantia_map):
    ...
    # Debug LINHA values from 2024 data
    linhas = []
    if "LINHA" in df.columns:
        linhas = df["LINHA"].dropna().unique().tolist()

    # Optional: Filter for Dispensers if needed
    # For now, we want to SEE what "LINHA" values exist for 2024
    # mask_dispensers = df["LINHA"].astype(str).str.upper().str.contains("DISPENSER", na=False)
    # df = df[mask_dispensers]
    ...
    samples = (
        df[["LINHA", "ITEM", "DESCRICAO", "QUANTIDADE", "RTM_STATUS"]]
        .head(2)
        .to_dict("records")
    )

    return df[["IS_RTM", "ELEC_ACTIVE", "NORM_ACTIVE", "QUANTIDADE"]], samples, linhas
...
def main():
    ...
    try:
        ...
        for i, chunk in enumerate(reader):
            ...
            result, chunk_samples, chunk_linhas = process_chunk(
                chunk, rtm_map, garantia_map
            )

            if result is not None:
                samples_collected.extend(chunk_samples)
                unique_linhas.update(chunk_linhas)
    ...
    print("\n" + "=" * 50)
    print(f"Unique LINHA values in 2024: {sorted(list(unique_linhas))}")
    print("\n" + "=" * 50)
    print("SAMPLES (2024):")
    for s in samples_collected[:5]:
        print(s)
    ...
    for category in ["RTM", "NON_RTM"]:
        ...
        total = data["total"]
        print(f"\nCategory: {category}")
        print(f"Total Chassis: {int(total)}")
        if total > 0:
