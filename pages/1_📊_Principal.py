...
def render_customer_relationship_section(df: pd.DataFrame):
    ...
    matrix_data, _, _ = vz.get_customer_relationship_matrix_data(df)
    if matrix_data.empty:
        st.info("Sem dados válidos para montar a matriz com os filtros atuais.")
        return

    opcoes_mantenedor = ["Todos"] + sorted(matrix_data["MANTENEDOR"].tolist())
    manter_destaque = st.selectbox(
        "Encontrar Mantenedor",
        options=opcoes_mantenedor,
        index=0,
        key="crm_mantenedor_destaque",
    )
    destaque = None if manter_destaque == "Todos" else manter_destaque

    fig_matrix = vz.customer_relationship_matrix_chart(df, destaque)
    if fig_matrix is not None:
        st.plotly_chart(fig_matrix, width="stretch")
    else:
        st.info("Não foi possível montar a matriz de relacionamento.")

    fig_monthly = vz.customer_relationship_monthly_score_chart(df)
