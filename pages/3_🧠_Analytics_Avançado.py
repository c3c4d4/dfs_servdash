...
st.set_page_config(
    page_title="Analytics Avançado - EM DESENVOLVIMENTO", layout="wide", page_icon="🧠"
)
...
if analysis_type == "Predição de Falhas":
    ...
    if len(features_df) > 0:
        ...
        if "error" not in model_results:
            ...
            # Predictions
            predictions_df = aa.failure_predictor.predict_failures(
                features_df, model_results
            )
            ...
            if len(predictions_df) > 0:
                ...
                high_risk = predictions_df[
                    predictions_df["RISK_CATEGORY"].isin(["Alto", "Crítico"])
                ]
    ...
    with col1:
        start_date = st.date_input(
            "Data Início", value=datetime.now() - timedelta(days=730)
        )
    with col2:
    ...
    chamados_filtered["DATA"] = pd.to_datetime(
        chamados_filtered["DATA"], errors="coerce"
    )
    ...
    if len(chamados_filtered) > 0:
        ...
        if len(ts_data) > 0:
            ...
            with st.spinner("Detectando padrões sazonais..."):
                seasonality = aa.time_series_analyzer.detect_seasonality(
                    ts_data, metric_for_season
                )
            ...
            if seasonality and seasonality.get("has_seasonality", False):
                ...
                col3.metric(
                    "Força Sazonal", f"{seasonality['seasonality_strength']:.2f}"
                )
                ...
                fig_season.update_layout(
                    ...
                    xaxis_title="Mês",
                    yaxis_title=metrics_labels.get(
                        metric_for_season, metric_for_season
                    ),
                    height=400,
                )
                ...
                st.info(
                    "ℹ️ Não foram detectados padrões sazonais significativos para esta métrica."
                )
            ...
            with st.spinner("Gerando previsões..."):
                forecast_data = aa.time_series_analyzer.forecast_metric(
                    ts_data, forecast_metric, forecast_periods
                )
            ...
            if len(forecast_data) > 0:
                ...
                forecast_display["Valor Previsto"] = forecast_display[
                    "Valor Previsto"
                ].round(1)
            ...
            col4.metric(
                "Taxa Média Garantia", f"{ts_data['TAXA_GARANTIA'].mean():.1f}%"
            )
        else:
    ...
    if len(corr_matrix) > 0:
        ...
        strong_corr = aa.correlation_analyzer.find_strong_correlations(
            ...
            for i, row in top_correlations.iterrows():
                ...
                if var1 in analysis_df.columns and var2 in analysis_df.columns:
                    ...
                    fig_scatter.update_layout(
                        showlegend=False, xaxis_title=var1, yaxis_title=var2
                    )
            ...
            with st.expander("💡 Interpretação das Correlações", expanded=True):
                ...
                **Correlações negativas**: Quando uma variável aumenta, a outra tende a diminuir.
                ...
                for _, row in top_correlations.iterrows():
                    ...
                    st.info(
                        f"**{var1}** e **{var2}**: {interpretation} (r={corr_val:.3f})"
                    )
        else:
    ...
    with st.spinner("Analisando padrões de manutenção..."):
        maintenance_analysis = aa.maintenance_optimizer.analyze_maintenance_patterns(
            chamados_df
        )
    ...
    if maintenance_analysis:
        ...
        col2.metric(
            "Serviços Preventivos", f"{maintenance_analysis['preventive_count']:,}"
        )
        col3.metric("Serviços Reativos", f"{maintenance_analysis['reactive_count']:,}")
        col4.metric(
            "Taxa Preventiva", f"{maintenance_analysis['preventive_ratio']:.1f}%"
        )
        ...
        if len(maintenance_analysis["service_analysis"]) > 0:
            ...
            recommendations = aa.maintenance_optimizer.recommend_maintenance_schedule(
            ...
            if len(recommendations) > 0:
                ...
                if len(filtered_recommendations) > 0:
                    st.dataframe(
                        ...
                        column_config={
                            "Frequência Atual": st.column_config.NumberColumn(
                                "Frequência Atual", format="%.2f"
                            ),
                            "Score de Impacto": st.column_config.NumberColumn(
                                "Score de Impacto", format="%.1f"
                            ),
                        },
                        use_container_width=True,
                        hide_index=True,
                    ...
                    roi_data = aa.maintenance_optimizer.calculate_maintenance_roi(
                        maintenance_analysis
                    )
                        ...
                        cost_comparison = pd.DataFrame(
                            {
                                ...
                                "Preventiva": [
                                    roi_data["current_preventive_cost"],
                                    roi_data["optimized_preventive_cost"],
                                ],
                                "Reativa": [
                                    roi_data["current_reactive_cost"],
                                    roi_data["optimized_reactive_cost"],
                                ],
                            }
                        )

                        fig_cost = px.bar(
                            cost_comparison.melt(
                                id_vars="Cenário", var_name="Tipo", value_name="Custo"
                            ),
                            x="Cenário",
                            y="Custo",
                            color="Tipo",
                    ...
                    calendar_data = (
                        aa.maintenance_optimizer.create_maintenance_calendar(
                    ...
                    if len(calendar_data) > 0:
                        ...
                        st.dataframe(
                            next_6_months,
                            column_config={
                                "Data Planejada": st.column_config.DateColumn(
                                    "Data Planejada"
                                )
                            },
                            use_container_width=True,
                        ...
                        calendar_counts = (
                            calendar_data.groupby("Data Planejada")
                            .size()
                            .reset_index(name="Quantidade")
                        )
                        calendar_counts["Data Planejada"] = pd.to_datetime(
                            calendar_counts["Data Planejada"]
                        )
    ...
    st.markdown(
        "**Previsões avançadas de KPIs e métricas de performance usando Machine Learning**"
    )
    ...
    with st.spinner("Preparando dados para previsão de performance..."):
        forecast_data = aa.performance_forecaster.prepare_forecast_data(
            chamados_df, bombas_df
        )
    ...
    if forecast_data and "weekly_performance" in forecast_data:
        ...
        if len(weekly_data) > 0:
            ...
            with st.spinner("Gerando previsões com Machine Learning..."):
                forecasts = aa.performance_forecaster.forecast_performance_metrics(
                    weekly_data, forecast_weeks
                )

            # Performance insights
            insights = aa.performance_forecaster.performance_insights(
                weekly_data, forecasts
            )
            ...
            if len(recent_data) > 0:
                perf_cols[0].metric(
                    ...
                    f"{insights.get('TOTAL_CHAMADOS', {}).get('pct_change', 0):+.1f}%"
                    if insights.get("TOTAL_CHAMADOS")
                    else "",
                )
                perf_cols[1].metric(
                    "🛡️ Taxa Garantia",
                    f"{recent_data['TAXA_GARANTIA'].mean():.1f}%",
                    f"{insights.get('TAXA_GARANTIA', {}).get('pct_change', 0):+.1f}%"
                    if insights.get("TAXA_GARANTIA")
                    else "",
                )
                perf_cols[2].metric(
                    "📡 Taxa RTM",
                    f"{recent_data['TAXA_RTM'].mean():.1f}%",
                    f"{insights.get('TAXA_RTM', {}).get('pct_change', 0):+.1f}%"
                    if insights.get("TAXA_RTM")
                    else "",
                )
                perf_cols[3].metric(
                    "⚡ Chamados/Bomba",
                    f"{recent_data['CHAMADOS_POR_BOMBA'].mean():.2f}",
                    f"{insights.get('CHAMADOS_POR_BOMBA', {}).get('pct_change', 0):+.1f}%"
                    if insights.get("CHAMADOS_POR_BOMBA")
                    else "",
                )

            # Forecast charts
            if len(forecasts) > 0:
                charts = aa.performance_forecaster.create_performance_dashboard(
                    weekly_data, forecasts
                )
                ...
                for metric, chart in charts.items():
                    if not show_confidence:
                        ...
                        chart.data = [
                            ...
                            if "Confiança" not in trace.name
                        ]
                ...
                if "forecasts" in insights:
                    ...
                    with insight_cols[0]:
                        ...
                        for metric, forecast_data in insights["forecasts"].items():
                            trend_icon = (
                                "📈"
                                if forecast_data["forecast_trend"] == "Crescente"
                                else "📉"
                            )
                            metric_name = {
                                "TOTAL_CHAMADOS": "Total de Chamados",
                            ...
                            st.info(
                                f"{trend_icon} **{metric_name}**: {forecast_data['forecast_trend']} (Média prevista: {forecast_data['forecast_avg']:.1f})"
                            )
                    ...
                    with insight_cols[1]:
                        ...
                        for metric, forecast_data in insights["forecasts"].items():
                            if (
                                metric == "TOTAL_CHAMADOS"
                                and forecast_data["forecast_avg"]
                                > weekly_data["TOTAL_CHAMADOS"].mean() * 1.2
                            ):
                                alerts.append(
                                    "🔴 Volume de chamados pode aumentar significativamente"
                                )
                            elif (
                                metric == "TAXA_GARANTIA"
                                and forecast_data["forecast_avg"] > 40
                            ):
                                alerts.append(
                                    "🟡 Taxa de garantia pode subir acima do normal"
                                )
                            elif (
                                metric == "CHAMADOS_POR_BOMBA"
                                and forecast_data["forecast_avg"] > 0.5
                            ):
                                alerts.append(
                                    "🟠 Eficiência pode diminuir (mais chamados por bomba)"
                                )
                ...
                export_forecasts["DATA"] = export_forecasts["DATA"].dt.strftime(
                    "%Y-%m-%d"
                )
                export_forecasts["VALOR_PREVISTO"] = export_forecasts[
                    "VALOR_PREVISTO"
                ].round(2)
        ...
        for metric in selected_metrics:
            with cols[col_idx % len(cols)]:
                if metric == "Volume de Chamados":
                    ...
                    garantia_count = (
                        chamados_filtered["SERVIÇO"]
                        .str.contains("GARANTIA", na=False)
                        .sum()
                    )
                    taxa_garantia = (
                        garantia_count / len(chamados_filtered) * 100
                        if len(chamados_filtered) > 0
                        else 0
                    )
                    ...
                    taxa_rtm = (
                        rtm_count / len(chamados_filtered) * 100
                        if len(chamados_filtered) > 0
                        else 0
                    )
                    ...
                    if len(monthly) > 0:
                        fig = px.line(
                            monthly, x="Mês", y="Chamados", title="Evolução Mensal"
                        )
                        st.plotly_chart(fig, use_container_width=True)
